...
const Container = styled(Link)`
	...
	box-shadow: 5px 5px 0px 0px ${({theme}) => theme.colorBackground};
	overflow: hidden;
`;
...
const Title = styled.h2`
	...
	background-color: ${({theme}) => theme.colorBackgroundWhite};
	font-size: 0.9rem;
	z-index: 2;
`;
...
const Tech = styled.span`
	...
	padding: 3px 7px;
	border: 1px solid;
	border-color: ${({theme}) => theme.colorBackground};
	border-radius: 5px;
	height: fit-content;
	background-color: ${({theme}) => theme.colorBackgroundLight};
	box-shadow: 1px 1px 0px 0px ${({theme}) => theme.colorBackground};
